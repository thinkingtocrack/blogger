  ...
  return (
    <div>
       ...
       <p>{new Date(post.updatedAt).toString().slice(16,24)}</p>
        ...
        {
          Array.isArray(post.photos) && post.photos.map((photo, index) => (
            <img key={index} src={IF+photo} className="w-52  mx-auto mt-8" alt=""/>
          ))
        }
        </div>
         <p className="mx-auto mt-8 font-medium text-lg">{post.desc}</p>
         <div className="flex items-center mt-8 space-x-4 font-semibold">
          <p>Categories:</p>
          <div className="flex justify-center items-center space-x-2">
          {post.categories?.map((c,i)=>(
            <div key={i} className="bg-gray-300 rounded-lg px-3 py-1 border-2 border-gray-400 shadow-sm">{c}</div>
          ))}
